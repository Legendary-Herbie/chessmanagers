...
export const RATING_CATEGORIES = ['blitz', 'rapid', 'classical'];
...
export const ratingHistoryQuerySchema = z.object({
    category: z.enum(['blitz', 'rapid', 'classical', 'all']).optional(),
...
export const matchListQuerySchema = z.object({
    q: z.string().trim().max(100).optional(),
    ratingCategory: ratingCategorySchema.optional(),
...
export const createMatchSchema = z.object({
    ...
    ratingCategory: ratingCategorySchema,
...
export const createTournamentSchema = z.object({
    ...
    ratingCategory: ratingCategorySchema.default('rapid'),
    isRated: z.boolean().default(true),
}).strict();

export const updateTournamentSchema = z.object({
    tiebreaks: z.array(z.enum(['buchholz', 'sonnebornBerger', 'directHeadToHead'])).max(3).refine(values => new Set(values).size === values.length, 'Tiebreaks must be unique.').optional(),
...
export const ratingCategorySettingsSchema = z.object({
...
const clubWebsiteSchema = z.string()
    ...
    .url('Enter a complete website address beginning with http:// or https://.')
