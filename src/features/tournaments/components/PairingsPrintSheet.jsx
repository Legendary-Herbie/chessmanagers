...
export default function PairingsPrintSheet({ tournament, participants, round, standings = [], mode = 'pairings' }) {
    ...
    return createPortal(<div className="tournament-print-sheet" aria-hidden="true">
        ...
        {mode === 'standings' ? <table><thead><tr><th>Rank</th><th>Player</th><th>Points</th></tr></thead><tbody>{standings.map(row => <tr key={row.playerId}><td>{row.rank}</td><td>{row.playerName}</td><td>{row.matchPoints}</td></tr>)}</tbody></table> : <table>
